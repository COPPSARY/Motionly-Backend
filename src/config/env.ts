...
const schema = z.object({
  ...
  LOG_LEVEL: logLevel,
  AI_PROVIDER: z.enum(['gemini', 'openai', 'anthropic', 'sp-cambodia', 'clauderouter', 'hashn0de']).default('gemini'),
  AI_MODEL: z.string().min(1),
  GEMINI_API_KEY: z.string().min(1).optional(),
  OPENAI_API_KEY: z.string().min(1).optional(),
  ANTHROPIC_API_KEY: z.string().min(1).optional(),
  SP_CAMBO_API_KEY: z.string().min(1).optional(),
  CLAUDEROUTER_API_KEY: z.string().min(1).optional(),
  HASHN0DE_API_KEY: z.string().min(1).optional(),
...
export function parseEnvironment(source: NodeJS.ProcessEnv | Record<string, string | undefined>) {
  ...
  return {
    ...
    claudeRouterApiKey: parsed.CLAUDEROUTER_API_KEY,
    hashn0deApiKey: parsed.HASHN0DE_API_KEY,
