...
function context(): GenerationJobContext {
  return {
    job: {
      ...
      baseSourceHash: 'base-source-hash', baseRevision: 1,
      outputSourceHash: null, retriedFromId: null, provider: 'gemini', model: 'fake-model',
...
function fakeStore(jobContext = context()) {
  ...
  const store: WorkerGenerationStore = {
    ...
    publish: vi.fn().mockResolvedValue({ status: 'COMPLETED', sourceHash: 'output-source-hash', revision: 2 }),
