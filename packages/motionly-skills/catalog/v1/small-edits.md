...
- Apply the smallest exact patch with `apply_project_patch`. Do not replace an entire file for a one-property change.
...
- Run source checks and submit only after confirming the requested change is present and unrelated source is unchanged.
